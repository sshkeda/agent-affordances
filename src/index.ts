...
export type AffordanceDefinition<Input extends Schema = Schema, Output extends Schema = Schema> =
...
export interface AffordanceCatalog<
  TAffordances extends readonly AnyAffordanceDefinition[] = readonly AnyAffordanceDefinition[],
> {
  id: string
  title?: string
  version?: string
...
function validateAffordanceDefinition(definition: AnyAffordanceDefinition): void {
...
export function defineTool<Input extends Schema, Output extends Schema>(
  ...
  validateAffordanceDefinition(definition as unknown as AnyAffordanceDefinition)
...
export function defineCatalog<TAffordances extends readonly AnyAffordanceDefinition[]>(
  ...
  return Object.freeze({
    ...catalog,
    affordances: Object.freeze([...catalog.affordances]) as unknown as TAffordances,
  })
}

export function catalogAffordances(catalog: AffordanceCatalog): AnyAffordanceDefinition[] {
...
export function catalogCommands(catalog: AffordanceCatalog): AnyCommandDefinition[] {
  return catalog.affordances.filter(
    (affordance): affordance is AnyCommandDefinition => affordance.kind === "command",
  )
}
...
export function toMcpTool(tool: AnyAffordanceDefinition): McpToolProjection {
  return {
    ...
    annotations: {
      ...(tool.execution?.sideEffects === "none" || tool.execution?.sideEffects === "read"
        ? { readOnlyHint: true }
        : {}),
      ...(tool.execution?.sideEffects === "write" || tool.execution?.sideEffects === "external"
...
export function toMcpTools(tools: AnyAffordanceDefinition[]): McpToolProjection[] {
...
export function toPiToolDefinition(tool: AnyAffordanceDefinition): PiToolProjection {
  return {
...
export function toPiToolDefinitions(tools: AnyAffordanceDefinition[]): PiToolProjection[] {
...
export function httpPathFor(tool: AnyAffordanceDefinition): string {
  return tool.projections?.openapi?.path ?? `/${defaultProjectionName(tool.id).replace(/_/g, "/")}`
}

export function toOpenApiOperation(tool: AnyAffordanceDefinition): OpenApiOperationProjection {
  ...
  return {
    ...
    method,
    operation: {
      operationId: tool.projections?.openapi?.operationId ?? defaultProjectionName(tool.id),
      summary: tool.title,
      description: tool.description,
      ...(tool.projections?.openapi?.tags ? { tags: tool.projections.openapi.tags } : {}),
      requestBody:
        method === "get"
          ? undefined
          : {
...
export function toSdkOperation(tool: AnyAffordanceDefinition): SdkOperationProjection {
  return {
    id: tool.id,
    name:
    ...
    ...(tool.execution?.sideEffects !== undefined
      ...
      : {}),
    ...(tool.execution?.idempotent !== undefined ? { idempotent: tool.execution.idempotent } : {}),
  }
}

export function toSdkOperations(tools: AnyAffordanceDefinition[]): SdkOperationProjection[] {
...
export function toCliCommand(tool: AnyAffordanceDefinition): CliCommandProjection {
  return {
    id: tool.id,
    name:
      tool.projections?.cli?.name ?? tool.projections?.sdk?.name ?? defaultProjectionName(tool.id),
    title: tool.projections?.cli?.title ?? tool.title,
    description: tool.projections?.cli?.description ?? tool.description,
    ...(tool.projections?.cli?.usage !== undefined ? { usage: tool.projections.cli.usage } : {}),
    ...(tool.projections?.cli?.group !== undefined ? { group: tool.projections.cli.group } : {}),
    ...(tool.projections?.cli?.examples !== undefined
      ? { examples: tool.projections.cli.examples }
      : {}),
    ...(tool.projections?.cli?.hidden !== undefined ? { hidden: tool.projections.cli.hidden } : {}),
    inputSchema: schemaToJsonSchema(tool.input),
    outputSchema: schemaToJsonSchema(tool.output),
  }
}

export function toCliCommands(tools: AnyAffordanceDefinition[]): CliCommandProjection[] {
...
export function toMarkdown(tool: AnyAffordanceDefinition): string {
  ...
  if (tool.permissions?.requires?.length)
    lines.push(`- permissions: ${tool.permissions.requires.map((p) => `\`${p}\``).join(", ")}`)
  if (tool.runtime?.env?.length)
    lines.push(`- env: ${tool.runtime.env.map((name) => `\`${name}\``).join(", ")}`)
  lines.push(
    "",
    "## Input schema",
    "",
