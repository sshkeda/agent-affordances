...
export interface ToolDefinition<Input extends Schema = Schema, Output extends Schema = Schema> {
  ...
  examples?: { name: string; input: z.input<Input>; output?: z.output<Output> }[]
}
...
export function toOpenApiOperation(tool: AnyToolDefinition): OpenApiOperationProjection {
  const method = tool.projections?.openapi?.method ?? "post"
  const path =
    tool.projections?.openapi?.path ?? `/${defaultProjectionName(tool.id).replace(/_/g, "/")}`
  return {
    path,
    method,
    operation: {
      ...
      requestBody:
        method === "get"
          ...
          : {
              required: true,
              content: { "application/json": { schema: schemaToJsonSchema(tool.input) } },
            },
      responses: {
        "200": {
          description: "Successful response",
          content: { "application/json": { schema: schemaToJsonSchema(tool.output) } },
        },
      },
    },
  }
...
): { openapi: "3.1.0"; info: OpenApiDocumentInfo; paths: Record<string, Record<string, unknown>> } {
